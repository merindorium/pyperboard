...
class RequestAttributePattern(Pattern):
    ATTRIBUTE_TYPES = {"?": "optional", "!": "required"}
    ...
    def _get_attribute_elem(
        self, attribute_name: str, attribute_type: str
    ) -> etree.Element:
...
class ApiEndpointPattern(Pattern):
    ...
    def _get_endpoint_elem(
        self, endpoint_method: str, endpoint_url: str
    ) -> etree.Element:
        elem = etree.Element("div")
...
class RestApiExtension(Extension):
    ...
    def extendMarkdown(self, md, md_globals):
        md.inlinePatterns.add(
            "attribute_field",
            RequestAttributePattern(self.REQUEST_ATTRIBUTE_RE, md),
            "_end",
        )
        md.inlinePatterns.add(
            "api_endpoint", ApiEndpointPattern(self.API_ENDPOINT_RE, md), "_end"
        )
        md.inlinePatterns.add(
            "socket_events", WebsocketEventPattern(self.SOCKET_EVENT_RE, md), "_end"
        )
        md.inlinePatterns.add(
            "value_list", ValuesListPattern(self.VALUES_LIST_RE, md), "_end"
        )


class SegmentPreprocessor(Preprocessor):
    SEGMENT_RE = re.compile(
        r"@@@(?P<segment_group>[\S]+?)\|(?P<segment_name>[\S]+?)\n(?P<data>[\s\S]+?)?\n@@@",
        re.MULTILINE | re.DOTALL | re.VERBOSE,
    )
    SEGMENT_CLASS = "segment"
    SEGMENT_WRAP = (
        '<div id="{segment_group}__{segment_name}" '
    ...
    def run(self, lines):
        ...
        while True:
            ...
            if matched:
                ...
                segment = self.SEGMENT_WRAP.format(
                ...
                text = "{}\n{}\n{}".format(
                    text[: matched.start()], segment, text[matched.end() :]
                )
...
class SegmentExtension(Extension):
    ...
    def extendMarkdown(self, md, md_globals):
        ...
        md.inlinePatterns.add(
            "segment_menu", SegmentMenuPattern(self.SEGMENT_MENU_RE, md), "_end"
        )
